...
//        setPaint(paint, false, 0, 0);
    ...
    private boolean drawImage(Image img, Image mask, AffineTransform xform, Color bgColor, ImageObserver obs) {
        ...
        try {
            com.extpdf.pdfutil.Image image = null;
            if(!convertImagesToJPEG){
                image = com.extpdf.pdfutil.Image.getInstance(img, bgColor);
            }
            else{
                ...
                scaled.flush();
                scaled = null;
                image = com.extpdf.pdfutil.Image.getInstance(baos.toByteArray());
                
            }
            if (mask!=null) {
                com.extpdf.pdfutil.Image msk = com.extpdf.pdfutil.Image.getInstance(mask, null, true);
                msk.makeMask();
    ...
    private void setPaint(boolean invert, double xoffset, double yoffset, boolean fill) {
        if (paint instanceof Color) {
            ...
            try {
                ...
                com.extpdf.pdfutil.Image image = com.extpdf.pdfutil.Image.getInstance(img, null);
